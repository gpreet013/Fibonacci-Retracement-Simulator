...
# =========================
# TIME EXIT CONFIG (by timeframe)
# =========================
def normalize_timeframe(tf: str) -> str:
    return (tf or "").strip().lower().replace(" ", "")

def get_time_exit_for_timeframe(tf: str) -> Tuple[int, int]:
    """
    Exit time rules (your requirement):
    - 5min  -> 15:15
    - 10min -> 15:10
    - 15min -> 15:00   ✅ (your request)
    - 30min -> 15:00
    """
...
def get_time_exit_minutes(tf: str) -> int:
    hh, mm = get_time_exit_for_timeframe(tf)
    return hh * 60 + mm
...
class FibonacciLeg:
    """
    One Fibonacci leg:
      - UPTREND: anchor = LOW pivot, extreme = HIGH pivot
      - DOWNTREND: anchor = HIGH pivot, extreme = LOW pivot
    """
    ...
    def __init__(
        ...
        self.entry_seq: Optional[int] = None
        self.entry_price: Optional[float] = None
        self.entry_dt: Optional[pd.Timestamp] = None
        ...
        self.time_exit_seq: Optional[int] = None
        self.time_exit_price: Optional[float] = None
        ...
        self.extreme_pivot_count = 0
        self.locked_extreme = False
        self.locked_extreme_label: Optional[str] = None
    ...
    def _dir(self) -> int:
        ...
        return 1 if self.trend == "UPTREND" else -1

    def get_exit_info(self) -> Tuple[Optional[int], Optional[float], Optional[str]]:
        """
        Returns exit seq, exit price, exit type (TARGET/STOPLOSS/TIME_EXIT) if trade closed.
        """
        if self.entry_seq is None or self.entry_price is None:
            return (None, None, None)

        if self.state == FibState.TARGET_HIT and self.target_seq is not None:
            return (int(self.target_seq), float(self.target_price), "TARGET")

        if self.state == FibState.STOPLOSS_HIT and self.stoploss_seq is not None:
            return (int(self.stoploss_seq), float(self.stoploss_price), "STOPLOSS")

        if self.state == FibState.TIME_EXIT and self.time_exit_seq is not None:
            return (int(self.time_exit_seq), float(self.time_exit_price), "TIME_EXIT")

        return (None, None, None)

    def trade_pnl_points(self) -> Optional[float]:
        """
        PnL points (positive = profit) for closed trades only.
        PnL = (exit - entry) * dir
        """
        exit_seq, exit_price, exit_type = self.get_exit_info()
        if exit_seq is None or exit_price is None:
            return None
        return (float(exit_price) - float(self.entry_price)) * self._dir()
    ...
    def _stoploss_close_or_cross0(self, o: float, h: float, l: float, c: float, lvl_0: float, lvl_382: float) -> bool:
        zone_low = min(lvl_0, lvl_382)
        zone_high = max(lvl_0, lvl_382)
        close_in_zone = (zone_low <= c <= zone_high)
        crossed_0 = (l < lvl_0) if self.trend == "UPTREND" else (h > lvl_0)
        return close_in_zone or crossed_0
    ...
    def _golden_zone_touched_no_382_cross(self, o: float, h: float, l: float, c: float, lvl_382: float, lvl_618: float) -> bool:
        ...
        if REQUIRE_BODY_IN_GOLDEN_ZONE:
            body_hi = max(o, c)
            body_lo = min(o, c)
            return (body_lo <= gz_high) and (body_hi >= gz_low)

        return (l <= gz_high) and (h >= gz_low)

    def _is_time_exit_candle(self, candle_dt: Optional[pd.Timestamp]) -> bool:
        """
        Exit at/after timeframe-based exit time.
        Must be SAME date as ENTRY.
        """
        if candle_dt is None or self.entry_dt is None:
            return False
        if pd.isna(candle_dt) or pd.isna(self.entry_dt):
            return False
        if candle_dt.date() != self.entry_dt.date():
            return False

        exit_minutes = get_time_exit_minutes(RESAMPLE_TIMEFRAME)
        cur_minutes = candle_dt.hour * 60 + candle_dt.minute
        return cur_minutes >= exit_minutes
    ...
    def update_candle(
        ...
        is_pivot_high: bool = False,
        is_pivot_low: bool = False,
        candle_dt: Optional[pd.Timestamp] = None,
    ) -> bool:
        if self.state in (FibState.INVALIDATED, FibState.STOPLOSS_HIT, FibState.TARGET_HIT, FibState.TIME_EXIT):
            return False
        ...
        if SHIFT_ANCHOR_ON_LOWER_LOW_BEFORE_HIGH and self.opposite_seq is None:
            if self.trend == "UPTREND":
                ...
                if should_shift:
                    ...
                    self.target_seq = None
                    self.target_price = None
                    self.time_exit_seq = None
                    self.time_exit_price = None
                    self.invalidation_seq = None
                    self.invalidation_reason = None
                    self.quality_rejected = False
                    self.quality_rejection_reason = None
        ...
        if self.state in (FibState.ACTIVE, FibState.VALIDATED):
            ...
            if not self.validation_info["entered_golden_zone"]:
                if self._golden_zone_touched_no_382_cross(open_price, high, low, close, lvl_382, lvl_618):
                    ...
                    self.state = FibState.VALIDATED
                    self.updates.append({"seq": seq, "price": float((low + high) / 2),
                                         "event": "GOLDEN_ZONE_TOUCHED",
                                         "detail": "✓ VALID (touched 0.382–0.618 without crossing 0.382 wick)"})
            ...
            if self.validation_info["entered_golden_zone"]:
                ...
                # A) BEFORE ENTRY => INVALIDATED if CLOSE in 0–0.382 OR crossed 0
                if self.entry_seq is None:
                    if self._stoploss_close_or_cross0(open_price, high, low, close, lvl_0, lvl_382):
                        self.state = FibState.INVALIDATED
                        self.invalidation_seq = int(seq)
                        self.invalidation_reason = "Invalid BEFORE ENTRY: CLOSE in 0–0.382 OR crossed 0 after GZ"
                        self.updates.append({"seq": seq, "price": float(close), "event": "INVALIDATED",
                                             "detail": self.invalidation_reason})
                        return False
                ...
                if self.entry_seq is None and (lvl_786 is not None and lvl_1 is not None):
                    ...
                    if close_in_786_1 and not (same_as_anchor or same_as_opposite or same_as_extreme or same_as_gz):
                        self.entry_seq = int(seq)
                        self.entry_price = float(close)
                        self.entry_dt = pd.to_datetime(candle_dt) if candle_dt is not None else None
                        self.locked_extreme = True
                        self.updates.append({"seq": seq, "price": self.entry_price, "event": "ENTRY",
                                             "detail": "✓ ENTRY (CLOSE inside 0.786–1.0). Extreme locked after entry."})

                # C) AFTER ENTRY => STOPLOSS / TARGET / TIME EXIT
                if self.entry_seq is not None:
                    ...
                    if self.stoploss_seq is None:
                        if self._stoploss_close_or_cross0(open_price, high, low, close, lvl_0, lvl_382):
                            ...
                            self.state = FibState.STOPLOSS_HIT
                            self.updates.append({"seq": seq, "price": self.stoploss_price, "event": "STOPLOSS_HIT",
                                                 "detail": "✗ STOPLOSS (after ENTRY: CLOSE in 0–0.382 OR crossed 0)"})
                            return False
                    ...
                    # 3) TIME EXIT (timeframe based)
                    if self.stoploss_seq is None and self.target_seq is None:
                        if self._is_time_exit_candle(candle_dt):
                            self.time_exit_seq = int(seq)
                            self.time_exit_price = float(close)
                            self.state = FibState.TIME_EXIT
                            exit_str = minutes_to_hhmm(get_time_exit_minutes(RESAMPLE_TIMEFRAME))
                            self.updates.append({"seq": seq, "price": self.time_exit_price, "event": "TIME_EXIT",
                                                 "detail": f"⏱ EXIT at {exit_str} (timeframe={RESAMPLE_TIMEFRAME}) — no SL & no TGT found"})
                            return False
...
# =========================
# PnL SUMMARY AGGREGATION
# =========================
def compute_pnl_summary(legs: List[FibonacciLeg]) -> Dict[str, float]:
    trades = []
    for leg in legs:
        pnl = leg.trade_pnl_points()
        if pnl is None:
            continue
        trades.append(pnl)

    total_profit = sum(x for x in trades if x > 0)
    total_loss = sum(-x for x in trades if x < 0)  # positive magnitude
    net = sum(trades)

    win_count = sum(1 for x in trades if x > 0)
    loss_count = sum(1 for x in trades if x < 0)
    ...
    return {
        ...
        "total_profit_points": float(total_profit),
        "total_loss_points": float(total_loss),
...
def export_fib_summary_pdf(filename: str, symbol: str, timeframe: str, sym_df: pd.DataFrame, legs: List[FibonacciLeg]):
    ...
    valid_target_hit = sum(1 for x in legs if x.validation_info.get("is_valid_fib") and x.state == FibState.TARGET_HIT)
    valid_stoploss_hit = sum(1 for x in legs if x.validation_info.get("is_valid_fib") and x.state == FibState.STOPLOSS_HIT)
    valid_time_exit = sum(1 for x in legs if x.validation_info.get("is_valid_fib") and x.state == FibState.TIME_EXIT)
    invalid_count = len(invalid_legs)
    ...
    valid_no_sl_tgt = sum(
        ...
        if x.validation_info.get("is_valid_fib")
        and x.state in (FibState.VALIDATED, FibState.ACTIVE)
        and x.stoploss_seq is None and x.target_seq is None and x.time_exit_seq is None
    )
    ...
    exit_str = minutes_to_hhmm(get_time_exit_minutes(timeframe))
    ...
    c.drawString(2 * cm, y, f"Intraday Date: {intraday_date_str}")
    y -= 0.7 * cm
    c.drawString(2 * cm, y, f"Time Exit Rule: {exit_str}")
    ...
    c.drawString(2 * cm, y, f"Valid + Stoploss Hit: {valid_stoploss_hit}")
    y -= 0.6 * cm
    c.drawString(2 * cm, y, f"Valid + Time Exit ({exit_str}): {valid_time_exit}")
    y -= 0.6 * cm
    c.drawString(2 * cm, y, f"Valid but NO SL & NO TGT yet: {valid_no_sl_tgt}")
    y -= 0.6 * cm
    c.drawString(2 * cm, y, f"Invalid Fibonacci: {invalid_count}")
    ...
    y -= 1.0 * cm
    c.setFont("Helvetica-Bold", 12)
    c.drawString(2 * cm, y, "PnL (Points) [only closed trades: TARGET/SL/TIME_EXIT]:")

    y -= 0.7 * cm
    c.setFont("Helvetica", 12)
    c.drawString(2 * cm, y, f"Total Profit Points: {pnl['total_profit_points']:.2f}")
    y -= 0.6 * cm
    c.drawString(2 * cm, y, f"Total Loss Points: {pnl['total_loss_points']:.2f}")
    y -= 0.6 * cm
    c.drawString(2 * cm, y, f"Net Points: {pnl['net_points']:.2f}")
    y -= 0.6 * cm
    c.drawString(2 * cm, y, f"Win Count: {int(pnl['win_count'])} | Loss Count: {int(pnl['loss_count'])}")
    y -= 0.6 * cm
    c.drawString(2 * cm, y, f"Avg Win: {pnl['avg_win_points']:.2f} | Avg Loss: {pnl['avg_loss_points']:.2f}")
    ...
    for leg in legs_sorted[:90]:
        ...
        exs_seq = leg.time_exit_seq if leg.time_exit_seq is not None else None
        exs_dt = seq_to_dt.get(int(exs_seq)) if exs_seq is not None else None
        exs_time = exs_dt.strftime("%H:%M") if exs_dt is not None else "-"

        pnl_points = leg.trade_pnl_points()
        pnl_str = f"{pnl_points:+.2f}" if pnl_points is not None else "-"
        ...
        line = (
            f"Fib-{leg.leg_id} | anchor={anchor_time}@{leg.anchor_price:.2f} | "
            f"ext={ext_time}@{ext} | state={leg.state.name} | "
            f"GZ={gz_time} | ENTRY={ent_time} | SL={sls_time} | TGT={tgs_time} | EXIT={exs_time} | PnL={pnl_str}{lock}"
        )
        c.drawString(2 * cm, y, line)
        y -= 0.45 * cm
...
def simulate_live_fibonacci_professional(
    ...
    for idx in range(start_idx, len(sym_df) - pivot_period):
        ...
        candle_dt = pd.to_datetime(row["datetime"]) if "datetime" in sym_df.columns else None
        ...
        if current_leg is not None:
            alive = current_leg.update_candle(
                seq, o, h, l, c,
...
def plot_professional_fibonacci(df: pd.DataFrame, symbol: str, legs: List[FibonacciLeg], min_gap: int):
    ...
    for leg in display_legs:
        ...
        if leg.state == FibState.TARGET_HIT:
            ...
            opacity = 0.35
            dash = "dot"
        elif leg.state == FibState.TIME_EXIT:
            opacity = 0.65
            dash = "dashdot"
        ...
        # EXIT marker
        if leg.state == FibState.TIME_EXIT and leg.time_exit_seq is not None:
            fig.add_trace(
                go.Scatter(
                    x=[seq_to_datetime(leg.time_exit_seq)],
                    y=[leg.time_exit_price],
                    mode="markers+text",
                    marker=dict(symbol="square", size=16, color="orange", line=dict(color="white", width=2)),
                    text=["EXIT"],
                    textfont=dict(size=11, color="white"),
                    showlegend=False,
                )
    ...
    pnl = compute_pnl_summary(display_legs)
    exit_str = minutes_to_hhmm(get_time_exit_minutes(RESAMPLE_TIMEFRAME))

    fig.update_layout(
        title=dict(
            text=(
                f"{symbol} | TF={RESAMPLE_TIMEFRAME} | Exit={exit_str} | "
                f"Net={pnl['net_points']:.2f} | Profit={pnl['total_profit_points']:.2f} | Loss={pnl['total_loss_points']:.2f} | "
                f"W={int(pnl['win_count'])}/L={int(pnl['loss_count'])}"
            ),
            font=dict(color="white", size=16),
        ),
        xaxis=dict(title="Time", showgrid=True, gridcolor="rgba(128,128,128,0.15)", color="white", type="date"),
        yaxis=dict(title="Price", showgrid=True, gridcolor="rgba(128,128,128,0.15)", color="white"),
        plot_bgcolor="black",
        paper_bgcolor="black",
        font=dict(color="white"),
...
def main():
    try:
        ...
        for sym in symbols:
            ...
            pnl = compute_pnl_summary(legs)
            print(
                f"PnL Summary: Profit={pnl['total_profit_points']:.2f}, "
                f"Loss={pnl['total_loss_points']:.2f}, Net={pnl['net_points']:.2f}, "
                f"W={int(pnl['win_count'])}, L={int(pnl['loss_count'])}, "
                f"AvgWin={pnl['avg_win_points']:.2f}, AvgLoss={pnl['avg_loss_points']:.2f}"
            )
